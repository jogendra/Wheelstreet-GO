...
class CameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    @IBOutlet weak var enterKMManuallyButton: UIButton!
    ...
    override func viewDidLoad() {
        ...
        previewView.frame = bikeKMCameraView.frame
    ...
    @IBAction func didTapEnterKMManually(_ sender: Any) {
    }
...
//            let y = (1.0 - height) / 2.0
			let initialRectOfInterest = bikeKMCameraView.frame
                //CGRect(x: x, y: y, width: width, height: height)
			metadataOutput.rectOfInterest = initialRectOfInterest

            DispatchQueue.main.async {
    self.previewView.setRegionOfInterestWithProposedRegionOfInterest(initialRectOfInterest)
            }
		} else {
